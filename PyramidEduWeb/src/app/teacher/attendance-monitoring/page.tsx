...
export default function TeacherAttendanceSummary() {
  ...
  const fetchSummary = async (p = 1, s = "") => {
    setLoading(true);
    try {
      const res = await api.get(`/attendance/teacher/summary?page=${p}&limit=10&search=${s}`);
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h2 className="text-2xl font-bold tracking-tight">My Students' Attendance</h2>
      </div>
                    ...
                    <tr key={student.studentId} className="border-b hover:bg-muted/30 transition-colors">
                      ...
                      <td className="px-4 py-3 text-right">
                        <Button 
                          variant="secondary" 
                          size="sm" 
                          onClick={() => router.push(`/teacher/attendance-monitoring/${student.studentId}`)}
                        >
                          View
                        </Button>
                      </td>
